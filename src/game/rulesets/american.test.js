...
// --- Registry / UI catalog ---
{
  ...
  assert.ok(style);
  assert.equal(style.rulesetId, "allFives");
  ...
  assert.equal(styles.some((s) => s.id === "allFives"), false);
  ...
  section("registry + GAME_STYLES American maps to allFives; duplicate All Fives hidden");
}
