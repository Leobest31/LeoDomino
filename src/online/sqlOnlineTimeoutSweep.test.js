...
// Canonical executable cron: only the active migrations path — no WITHHELD draft required.
...
assert.doesNotMatch(cronSql, /WITHHELD/);

const cronMigrations = readdirSync(join(root, "supabase/migrations")).filter((name) =>
  /online_timeout_sweep_cron/.test(name)
);
assert.deepEqual(
  cronMigrations,
  ["20260904140000_online_timeout_sweep_cron.sql"],
  "exactly one canonical executable timeout-sweep cron migration"
);
assert.equal(
  existsSync(join(root, "supabase/held/20260904121000_online_timeout_sweep_cron.WITHHELD.sql")),
  false,
  "obsolete WITHHELD cron draft must not be required"
);
