...
assert.doesNotMatch(source, /player_global_ratings|match_rp_results|settle_match_global_rp|admin_list_top_rp|admin_list_player_rp_history/);
...

{
  ...
  assert.equal("rp" in leaked, false);
  assert.equal("wins" in leaked, false);
  assert.equal("losses" in leaked, false);
}
...

{
  const page = normalizeAdminUserList({
    users: [
      { player_id: "a", display_name: "Ada", username: "ada" },
    ],
    total: 41,
    limit: 25,
    offset: 0,
...

{
  ...
  assert.equal("rp" in live.playerA, false);
  assert.equal("rp" in live.playerB, false);
...

{
  const waiting = normalizeAdminLiveMatch({
    ...
    player_a: { player_id: "a", display_name: "A", username: "a", stale: false },
    player_b: { player_id: "b", display_name: "B", username: "b", stale: false },
    score_a: null,
    score_b: null,
    round: null,
    current_seat: null,
  ...
  assert.equal(waiting.stakePips, null);
}

{
  const staked = normalizeAdminLiveMatch({
  ...
  assert.equal(staked.stakePips, 20);
  const invalidStake = normalizeAdminLiveMatch({
  ...
  assert.equal(invalidStake.stakePips, null);
}
    ...
    matches: [
      {
        ...
        match_status: "playing",
        admin_status: "live",
        player_a: { player_id: "a", username: "a", stale: false },
        player_b: { player_id: "b", username: "b", stale: false },
        score_a: 0,
        score_b: 0,
        round: 1,
      },
